...
class AstraError(Exception):
    """Base for every error Astra raises deliberately."""
    ...
    def to_problem(self, *, trace_id: str | None = None) -> dict[str, Any]:
        ...
        problem: dict[str, Any] = {
            "type": f"https://astra.dev/errors/{self.code.value.lower().replace('_', '-')}",
...
class BudgetExceededError(AstraError):
    code = ErrorCode.BUDGET_EXCEEDED


class DeadlineExceededError(AstraError):
    code = ErrorCode.DEADLINE_EXCEEDED


class VerificationFailedError(AstraError):
    code = ErrorCode.VERIFICATION_FAILED


class IllegalTransitionError(AstraError):
...
class PolicyInvariantViolation(AstraError):
...
class PrivacyRoutingViolation(AstraError):
