...
machine lives in ``astra.runtime.state``, and the layering rules forbid the
security layer from reaching into the runtime; ``astra.runtime.gate`` is the
...
from astra.core.types import ApprovalStatus, Capability
from astra.store.models import Approval
...
class ApprovalService:
    ...
    async def create(self, request: ApprovalRequest) -> Approval:
        ...
        log.info(
            "astra.security.approval_requested",
    ...
    async def approve(
        ...
        *,
        ...
        await self._session.flush()
        log.info(
            "astra.security.approval_decided",
            approval_id=approval.id,
    ...
    async def reject(self, approval: Approval, *, decided_by: str) -> Approval:
        ...
        approval.decided_at = self._clock.now()
        await self._session.flush()
        log.info("astra.security.approval_rejected", approval_id=approval.id)
        return approval
    ...
    async def mark_expired(self, approval: Approval) -> Approval:
        ...
        await self._session.flush()
        log.info("astra.security.approval_expired", approval_id=approval.id)
        return approval
