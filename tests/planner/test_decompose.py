...
@pytest.mark.asyncio
async def test_mock_planner_multi_step_list_then_report(settings) -> None:
    registry = registry_with_fakes()
    catalog = ToolCatalog(registry).list()
    result = await decompose(
        "list D:/tmp/workspace then report findings",
        catalog=catalog,
        capability_ceiling=Capability.L2,
        settings=settings,
        registry=registry,
    ...
    assert result.plan.steps[0].actions[0].tool == "fs.list_dir"
    assert result.plan.steps[1].actions[0].tool == "task.report"
    assert result.plan.steps[1].depends_on == ["survey"]
