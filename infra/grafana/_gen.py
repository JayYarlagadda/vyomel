...
DASHBOARDS: list[tuple[str, str, list[tuple[str, str]]]] = [
    (
        ...
        [
            ...
            ("Task duration p95", "histogram_quantile(0.95, sum(rate(vyomel_task_duration_seconds_bucket[5m])) by (le, status))"),
            ("Replans", "sum(rate(vyomel_task_replans_total[5m]))"),
            ("Human interventions", "sum(rate(vyomel_human_interventions_total[5m])) by (reason)"),
            ("Cost per task", "histogram_quantile(0.50, sum(rate(vyomel_task_cost_usd_bucket[5m])) by (le))"),
            ("Steps per task", "histogram_quantile(0.50, sum(rate(vyomel_task_steps_bucket[5m])) by (le))"),
        ],
    ),
    (
        ...
        [
            ...
            ("Action duration p95", "histogram_quantile(0.95, sum(rate(vyomel_action_duration_seconds_bucket[5m])) by (le, tool))"),
    ...
    (
        ...
        "Vyomel Model Performance",
        [
            ("Model calls", "sum(rate(vyomel_model_calls_total[5m])) by (provider, model, purpose)"),
            ("TTFT p95", "histogram_quantile(0.95, sum(rate(vyomel_model_ttft_seconds_bucket[5m])) by (le, provider, model))"),
            ("Latency p95", "histogram_quantile(0.95, sum(rate(vyomel_model_latency_seconds_bucket[5m])) by (le, provider, model))"),
    ...
    (
        ...
        [
            ...
            ("Approval wait p95", "histogram_quantile(0.95, sum(rate(vyomel_approval_wait_seconds_bucket[5m])) by (le))"),
            ...
            ("L4 denials", 'sum(rate(vyomel_policy_decisions_total{capability="L4",decision="DENY"}[5m]))'),
        ],
    ),
    (
        "vyomel-memory",
        ...
        [
            ...
            ("Retrieval latency p95", "histogram_quantile(0.95, sum(rate(vyomel_retrieval_latency_seconds_bucket[5m])) by (le, strategy))"),
    ...
    (
        ...
        [
            ...
            ("Queue wait p95", "histogram_quantile(0.95, sum(rate(vyomel_action_queue_wait_seconds_bucket[5m])) by (le))"),
...
def main() -> None:
    for uid, title, specs in DASHBOARDS:
        ...
        path.write_text(json.dumps(_dashboard(uid, title, panels), indent=2) + "\n", encoding="utf-8")
        print(path.name)
