...
_IMMUTABLE_FN = """
CREATE OR REPLACE FUNCTION vyomel_audit_log_immutable() RETURNS trigger AS $$
BEGIN
...
def upgrade() -> None:
    ...
    op.execute(
        ...
        "FOR EACH ROW EXECUTE FUNCTION vyomel_audit_log_immutable()"
    )


def downgrade() -> None:
    op.execute("DROP TRIGGER IF EXISTS audit_log_immutable ON audit_log")
    op.execute("DROP FUNCTION IF EXISTS vyomel_audit_log_immutable()")
