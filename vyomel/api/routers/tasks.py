...
from vyomel.orchestrator.planning import create_task as create_planned_task
from vyomel.orchestrator.plans import PlanService
from vyomel.orchestrator.runtime import get_registry
from vyomel.orchestrator.tasks import TaskBounds, TaskProgressCounts, TaskService
from vyomel.orchestrator.trace import build_trace
...
async def get_trace(
    ...
    task = await service.get(task_id)
    tree = await build_trace(session, task)
    return TraceResponse(
        task_id=task.id,
        trace_id=task.trace_id,
        rendered=render_timeline(tree),
        tree=_trace_node(tree),
    )


def _trace_node(node: TraceNode) -> TraceNodeResponse:
    return TraceNodeResponse(
        name=node.name,
        status=node.status,
        duration_s=node.duration_s,
        detail=node.detail,
        excluded_from_task_time=node.excluded_from_task_time,
        children=[_trace_node(child) for child in node.children],
