...
from vyomel.orchestrator.plans import HandwrittenPlan
...
class WorkflowParameterOut(BaseModel):
    name: str
    description: str = ""
    example: Any = None
...
class WorkflowOut(BaseModel):
    ...
    pattern_key: str
    definition: list[WorkflowStepOut]
    parameters: list[WorkflowParameterOut]
...
class MineWorkflowsRequest(BaseModel):
    actions: list[dict[str, Any]] = Field(default_factory=list)
    min_support: int = Field(default=3, ge=1, le=100)


class MineWorkflowsResponse(BaseModel):
    proposals: list[WorkflowOut]
