...
It lives in the runtime rather than in ``vyomel.security`` because it is the one
...
reverse. ``vyomel.security`` stays free of scheduler concepts as a result.
...
from vyomel.core.types import ActionStatus, Capability, Decision, TaskStatus
from vyomel.runtime.state import ActionTrigger, TaskTrigger, apply_action, apply_task
from vyomel.security.approvals import ApprovalRequest, ApprovalService, parameter_hash
from vyomel.security.audit import AuditEvent, AuditTrail
from vyomel.security.policy import PolicyDecision, PolicyRequest, PolicyStore
...
class PolicyGate:
    ...
    async def check(
        ...
        from vyomel.obs.metrics import POLICY_DECISIONS
        from vyomel.obs.tracing import start_span

        with start_span("policy.evaluate") as span:
            span.set(
                **{
                    "policy.decision": decision.decision.value,
                    "policy.rule_id": decision.rule_id,
                    "policy.version_hash": decision.policy_hash,
                    "policy.capability": action.capability_level.value,
                }
            )
    ...
    async def _require_approval(
        ...
        from vyomel.obs.metrics import HUMAN_INTERVENTIONS
