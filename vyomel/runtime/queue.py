...
class ActionQueue:
    ...
    async def publish(self, action_id: str, *, traceparent: str | None = None) -> str:
        fields: dict[str, str] = {"action_id": action_id}
        if traceparent:
            fields["traceparent"] = traceparent
        message_id = await self._redis.xadd(self.stream, cast(Any, fields))
    ...
    async def depth(self) -> int:
        return int(await self._redis.xlen(self.stream))
...
def _parse(entry: object) -> StreamMessage | None:
    ...
    raw_parent = fields.get("traceparent")
    if isinstance(raw_parent, bytes):
        raw_parent = raw_parent.decode()
    return StreamMessage(
        message_id=str(message_id),
        action_id=str(action_id),
        traceparent=str(raw_parent) if raw_parent else None,
    )
