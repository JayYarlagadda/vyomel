...
from vyomel.tools.browser.fixture import a11y_to_dict
from vyomel.tools.browser.session import backend_name, get_session
...
class BrowserOpen(Tool):
    ...
    async def execute(self, params: BaseModel, ctx: ToolContext) -> BaseModel:
        ...
        snap = await _maybe_await(session.open(params.url))
...
class BrowserQuery(Tool):
    ...
    async def execute(self, params: BaseModel, ctx: ToolContext) -> BaseModel:
        ...
        settings = _settings(ctx)
        session = await get_session(settings, task_id=ctx.task_id)
        element = await _maybe_await(session.query(_target(**params.model_dump())))
        return QueryOutput(
            ref=element.ref,
            role=element.role,
            name=element.name,
...
class BrowserClick(Tool):
    ...
    async def execute(self, params: BaseModel, ctx: ToolContext) -> BaseModel:
        ...
        settings = _settings(ctx)
        session = await get_session(settings, task_id=ctx.task_id)
        payload = await _maybe_await(session.click(_target(**params.model_dump())))
...
class BrowserSelect(Tool):
    ...
    async def execute(self, params: BaseModel, ctx: ToolContext) -> BaseModel:
        ...
        session = await get_session(settings, task_id=ctx.task_id)
        payload = await _maybe_await(
            session.select(_target(**params.model_dump(exclude={"value"})), params.value)
        )
...
class BrowserScroll(Tool):
    ...
    async def execute(self, params: BaseModel, ctx: ToolContext) -> BaseModel:
        ...
        session = await get_session(settings, task_id=ctx.task_id)
        payload = await _maybe_await(
            session.scroll(direction=params.direction, amount=params.amount)
        )
...
class BrowserSubmit(Tool):
    ...
    async def execute(self, params: BaseModel, ctx: ToolContext) -> BaseModel:
        ...
        payload = await _maybe_await(
            session.submit(target if any(params.model_dump().values()) else None)
        )
...
class BrowserScreenshot(Tool):
    ...
    async def execute(self, params: BaseModel, ctx: ToolContext) -> BaseModel:
        ...
        payload = await _maybe_await(session.screenshot(path))
