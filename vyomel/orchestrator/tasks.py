...
from vyomel.core.types import ActionStatus, Capability, StepStatus, TaskOrigin, TaskStatus
from vyomel.obs.metrics import TASKS_TOTAL
from vyomel.obs.tracing import new_trace_id, start_span
from vyomel.runtime.cancel import Canceller, CancelReport
...
class TaskService:
    ...
    async def create(
        ...
        *,
        ...
        with start_span("task", trace_id=task.trace_id) as span:
            span.set(
                **{
                    "task.id": task.id,
                    "task.origin": origin.value,
                    "task.status": task.status.value,
                }
            )
            TASKS_TOTAL.labels(status=task.status.value, origin=origin.value).inc()
